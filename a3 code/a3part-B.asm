...
start:	
	timer3:
		;checking if the timer has reached top value
		in temp, TIFR3
		sbrs temp, OCF3A
		rjmp timer3 ;if has no reached top value, go through polling loop again

		;reset the bit if top value has been reached
		ldi temp, 1<<OCF3A 
		out TIFR3, temp
...
timer1: ;check if button is pushed or not
	...
	wait:
		...
		andi r16, 0x40
		brne wait
		lds lowdata, ADCL ;grabbing the data from button 
		lds highdata, ADCH

		;comparing bounds from buttons to check if pressed
		clr r23
		cp lowdata, lowbound
