...
start:	
	timer3: ;checking if timer has reached top value
		in temp, TIFR3
		sbrs temp, OCF3A
	...
	;function to display L,R,U,D for corresponding location on screen
		displayfunction:
			push r16
			push r17
			...
			push r30 ;saved char for corresponding button
			rcall lcd_putchar
			pop r30
...
timer1: ;check if button is pushed or not
	...
	wait:
		...
		andi r16, 0x40
		brne wait
		lds lowdata, ADCL ;load data from buttons
		lds highdata, ADCH

		clr r23
		;checking the bounds to see if button was pressed
		cp lowdata, lowbound
		cpc highdata, highbound ;if the value in data is same or higher than 901 (bound) branch
		brsh done		
		ldi r23, 1 ;if pressed store 1, else 0
		sts BUTTON_IS_PRESSED, r23
...
timer4:
	...
	lds temp, BUTTON_IS_PRESSED
	cpi temp, 0x01
	breq checkUorD ;if button is pressed check up and down button
	rjmp exit
	...
	checkUorD:
		...
		;scrolls up through the string
		uploop:
		ldi ZH, high(AVAILABLE_CHARSET<<1) 
		ldi ZL, low(AVAILABLE_CHARSET<<1) 		
		lds temp, CURRENT_CHARSET_INDEX
		inc temp ;inc to the next "index" of string
		ldi r19, 0 ;counter 

		 ;counter for loop
		miniloop:
			lpm r17, Z+ ;takes out char from stirng
			inc r19
			cp r19, temp ;once equal exit
			brne miniloop
			
		getout:
			sts CURRENT_CHARSET_INDEX, temp ;load correct char and index corresponding to char
			...
			;changes the char and index if out of bounds
			outofbounds:
				ldi temp, 0x30
				sts TOP_LINE_CONTENT, temp
				ldi temp, 0
		...
		;scrolls down through the string
		downloop:
		ldi ZH, high(AVAILABLE_CHARSET<<1) 
		ldi ZL, low(AVAILABLE_CHARSET<<1) 

		lds temp, CURRENT_CHARSET_INDEX ;taking current "index" of char we r on 
		dec temp ;dec to "index" we want to get to 
		ldi r19, 0 ;counter for loop
	
		cpi temp, -1 ;checking if out of bounds
		brne miniloop2

		;reseting if out of bounds to correct char and correspodning index
		test:
			ldi temp, 17
			sts CURRENT_CHARSET_INDEX, temp
			ldi temp, 0x5F
		...
		getout2:
			sts CURRENT_CHARSET_INDEX, temp ;stores char and corresponding index
			sts TOP_LINE_CONTENT, r17
