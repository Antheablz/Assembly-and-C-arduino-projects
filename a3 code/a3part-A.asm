...
start:	
	timer3:
		;looking to see eif the timer has hreached top value
		in temp, TIFR3
		sbrs temp, OCF3A
		rjmp timer3 ;if not top value, go through polling loop again
...
timer1: ;check if button is pushed or not
	...
	wait:
		...
		andi r16, 0x40
		brne wait
		lds lowdata, ADCL ;grabbing data from button
		lds highdata, ADCH
	
		;comparing the bounds from the buttons to check if pressed
		clr r23
		cp lowdata, lowbound
		cpc highdata, highbound
		brsh done	
		ldi r23, 1 ;if pressed load a 1 
		sts BUTTON_IS_PRESSED, r23
